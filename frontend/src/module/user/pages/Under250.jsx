...
export default function Under250() {
  ...
  const updateItemQuantity = (item, newQuantity, event = null, restaurant = null) => {
    ...
    // Extract info safely and ensure IDs are strings
    const rName = restaurant?.name || item.restaurant || "Restaurant"
    const rId = String(restaurant?.id || restaurant?._id || restaurant?.restaurantId || item.restaurantId || "")
    ...
    if (event) {
      const buttonElement = event.currentTarget || (event.target && event.target.closest('button'))
      if (buttonElement) {
    ...
    try {
      ...
      // Update local state ONLY after successful cart operation
      setQuantities((prev) => ({
        ...prev,
        [itemId]: newQuantity,
      }))
