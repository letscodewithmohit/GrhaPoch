...
export default function RestaurantDetails() {
  ...
  useEffect(() => {
    ...
    const normalizedCurrentName = (restaurant?.name || "").trim().toLowerCase();

    cart.forEach((item) => {
      const normalizedItemName = (item.restaurant || "").trim().toLowerCase();
      if (normalizedItemName === normalizedCurrentName) {
        // Use string ID as key for consistency
        cartQuantities[String(item.id)] = item.quantity || 0
      }
    })
  ...
  const updateItemQuantity = (item, newQuantity, event = null) => {
    ...
    const itemId = String(item.variantId || item.id);
    ...
    // Prepare cart item with all required properties
    // Ensure we have a valid restaurant identifier
    const finalRestaurantId = validRestaurantId || restaurant?.id || restaurant?._id;
