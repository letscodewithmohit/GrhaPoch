...
export const createOrUpdateFeeSettings = asyncHandler(async (req, res) => {
  try {
    ...
    if (req.body.fixedFee !== undefined && req.body.fixedFee < 0) {
      return errorResponse(res, 400, 'Fixed fee must be a positive number');
    }
    ...
    const feeSettingsData = {
      ...
      fixedFee: req.body.fixedFee !== undefined ? Number(req.body.fixedFee) : 0,
...
export const updateFeeSettings = asyncHandler(async (req, res) => {
  try {
    const { id } = req.params;
    const { deliveryFee, deliveryFeeRanges, freeDeliveryThreshold, platformFee, platformFeeRanges, gstRate, fixedFee, isActive } = req.body;
    ...
    if (fixedFee !== undefined) {
      ...
      feeSettings.fixedFee = Number(fixedFee);
    }
...
export const getPublicFeeSettings = asyncHandler(async (req, res) => {
  try {
    const feeSettings = await FeeSettings.findOne({ isActive: true })
      .sort({ createdAt: -1 })
      .select('deliveryFee freeDeliveryThreshold platformFee gstRate fixedFee')
      .lean();
