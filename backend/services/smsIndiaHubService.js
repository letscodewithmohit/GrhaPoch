...
class SMSIndiaHubService {
  ...
  async initializeCredentials() {
    const { getSMSHubIndiaCredentials } = await import('../utils/envService.js');
  ...
  async sendOTP(phone, otp, purpose = 'register') {
    try {
      ...
      if (customTemplate) {
        ...
        message = `Welcome to the GrhaPoch powered by SMSINDIAHUB. Your OTP for ${purposeText} is ${otp}`;
      } else {
